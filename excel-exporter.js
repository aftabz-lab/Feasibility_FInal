...
export async function downloadRulesWorkbook(
  ...
  const suggestedName = `${safeFileName(data?.project?.locationArea)}_with_rules.xlsx`;
  let fileHandle = null;
  ...
  if (!(baseBuffer instanceof ArrayBuffer) || baseBuffer.byteLength === 0) {
    const configuredWorkbook = await getConfiguredRulesWorkbook();
    baseBuffer = configuredWorkbook.buffer;
    resolvedSourceName = configuredWorkbook.sourceName;
  }

  // IMPORTANT: load the source/master as a template, then update it.
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(baseBuffer);

  // Rebuild/update the three report sheets using CURRENT dashboard state.
  await addScoreSheet(workbook, data, model, assets);
  await addInformationSheet(workbook, data, model, assets);
  await addFeasibilitySheet(workbook, data, model, assets);

  // Keep the three report sheets visible. Retain all dependencies but hide them.
  const visibleSheets = new Set([
    "Sales forecasting tools",
    "INFORMATION",
    "AUTO GENERATED FEASIBILITY",
  ...
  // ExcelJS must serialize the workbook. Never wrap the original source buffer
  // directly in a Blob after modifying the dashboard state.
  const outputBuffer = await workbook.xlsx.writeBuffer();
